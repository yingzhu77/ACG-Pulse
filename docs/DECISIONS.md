...
**重新判断**：情感结果只有在状态成功、版本匹配且分析时间未超过 30 天时才复用。失败、旧版本和过期结果在后续社区刷新中重新判断。DeepSeek 是未显式配置 `AI_PROVIDER` 时唯一默认 Provider；OpenRouter 和 MiMo 只能通过显式选择启用，避免密钥同时存在时静默切换。

**热度语义**：B站、NGA 和小黑盒先按各自可获得指标计算原始热度，再在每个来源内部按百分位映射到 10–100。主页面和洞察中的热度表示本轮采集内的相对位置，不表示跨平台原始播放量、回复量或互动量的绝对等价。

**模块边界**：社区功能拆为来源采集、情感判断、热度计算、聚合编排和数据库持久化。`adapters/community.ts` 只负责聚合，Provider 解析由 `ai/providerConfig.ts` 统一拥有。

**任务保留**：`AnalysisTask` 成功记录默认保留 14 天，已耗尽重试机会的失败记录默认保留 30 天。仍可重试的失败任务不会被历史清理删除。服务启动和每日定时任务执行清理；运维接口暴露最近一次清理时间、数量和当前保留配置。
